...
### Q: macOS 双击 start.command 提示"无法打开" / "无法验证开发者"
macOS Gatekeeper 会阻止从网络下载的未签名脚本运行。解决方式（任选一种）：
...
**方式二：右键打开**
右键点击 `start.command` → 选择「打开」→ 弹窗中点击「打开」。macOS 会记住选择，之后双击也能运行。

**方式三：系统设置放行**
如果已经双击被拦截：打开「系统设置」→「隐私与安全性」→ 滚动到底部找到被拦截的提示 → 点击「仍要打开」。

> 如果是发给朋友使用，建议在压缩包外附上一句说明：首次运行请在终端执行 `chmod +x start.command && xattr -d com.apple.quarantine start.command`
